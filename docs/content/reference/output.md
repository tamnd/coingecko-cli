...
describes what users get. Pick a format with `-o`, or let coingecko choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
coingecko <command> -o table   # aligned columns for reading
coingecko <command> -o jsonl   # one JSON object per line, for piping
coingecko <command> -o json    # a single JSON array
coingecko <command> -o csv     # spreadsheet friendly
coingecko <command> -o tsv     # tab-separated
coingecko <command> -o url     # just the URL column
coingecko <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
coingecko <command> --fields id,title,url
```
...
```bash
coingecko <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
coingecko <command>            # a table, because this is a terminal
coingecko <command> | wc -l    # JSONL, because this is a pipe
```
