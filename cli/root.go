// Package cli assembles the coingecko command tree from the coingecko
...
// NewApp assembles the kit application from the coingecko domain. The
...
//
// To add a command, declare it in coingecko/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := coingecko.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(coingecko.Domain{}).Register(app)
